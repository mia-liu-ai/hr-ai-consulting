...
export default function App() {
  ...
  function goModule(module: Module) {
    ...
    if (adminModules.includes(module) && currentUser.role !== 'admin') {
      ...
      <div className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3">
          ...
          <nav className="-mx-1 flex max-w-full flex-wrap gap-1 overflow-x-auto px-1">
            {navButton('home', '组织咨询首页')}
            {currentUser?.role === 'admin' ? (
              <>
                {navButton('diagnosis', 'HR诊断假设')}
                {navButton('talent', 'AI人才模型')}
                {navButton('review360', '360评审Agent')}
                {navButton('admin', '管理员控制台')}
                {navButton('feedback', '员工反馈')}
                {navButton('orgDashboard', '组织诊断看板')}
                {navButton('diagnosisReports', '报告生成')}
              </>
            ) : null}
            {currentUser?.role === 'employee' ? (
              <>
                {navButton('employee', '我的评审任务')}
                {navButton('feedback', '员工反馈')}
              </>
            ) : null}
