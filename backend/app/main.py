...
def create_project(payload: ProjectPayload) -> dict[str, Any]:
    with get_connection() as conn:
        ...
        return fetch_one_or_404(conn, "SELECT * FROM projects WHERE id = ?", (project_id,), "project")


@app.get("/api/projects/{project_id}")
def get_project(project_id: int) -> dict[str, Any]:
    with get_connection() as conn:
        project = fetch_one_or_404(conn, "SELECT * FROM projects WHERE id = ?", (project_id,), "project")
...
def update_project(project_id: int, payload: ProjectPayload) -> dict[str, Any]:
    with get_connection() as conn:
        ...
        conn.execute(
            """
            UPDATE projects
            SET name = ?, purpose = ?, scope = ?, start_date = ?, end_date = ?,
...
def list_reports(project_id: int) -> list[dict[str, Any]]:
    with get_connection() as conn:
        rows = conn.execute(
            """
